...
const Carousel = ({
  ...
  return <StyledCarouselProvider sx={sx} step={step} spacing={spacing} interval={interval} infinite={infinite} isPlaying={autoPlay} totalSlides={totalSlides} currentSlide={currentSlide} visibleSlides={visibleSlides} hasMasterSpinner={hasMasterSpinner} isIntrinsicHeight={isIntrinsicHeight} naturalSlideWidth={naturalSlideWidth || 100} naturalSlideHeight={naturalSlideHeight || 125}>
      ...
      {showArrow && <Fragment>
          <StyledArrowBackButton id="backArrowButton" aria-label="Previous slide" sx={{
        left: "-20px"
          ...
          <StyledArrowNextButton id="backForwardButton" aria-label="Next slide" sx={{
        right: "-20px"
...
const renderDots = ({
  ...
  for (let i = 0; i < total; i += step) {
    dots.push(<StyledDot dot_color={dotColor} aria-label={`Go to slide ${i + 1}`} onClick={() => handleClick(i, false)} dot_active={currentSlide === i ? i + 1 : 0} key={(Math.random() * i + Date.now()).toString()} />);
    if (total - (i + 1) < step && total - (i + 1) !== 0) {
      dots.push(<StyledDot dot_color={dotColor} aria-label={`Go to slide ${totalSlides - visibleSlides + 1}`} dot_active={totalSlides - visibleSlides} key={(Math.random() * i + Date.now()).toString()} onClick={() => handleClick(totalSlides - visibleSlides, false)} />);
    }
  }
  return dots;
};
