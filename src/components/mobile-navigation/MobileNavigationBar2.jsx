...
const MobileNavigationBar2 = ({
  ...
  useEffect(() => {
    const listner = () => {
      if (window.scrollY > 30) setTotalHeight(mobileNavHeight);else setTotalHeight(total);
    };
    window.addEventListener("scroll", listner);
    return () => window.removeEventListener("scroll", listner);
  }, [mobileNavHeight, total]);
  // Always render and let CSS media queries handle visibility (Wrapper is hidden above 900px).
