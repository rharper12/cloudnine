...
const NavBarWrapper = styled(BazaarCard)(({
  theme,
  border
}) => ({
  height: "60px",
  ...
  const renderNestedNav = (list = [], isRoot = false) => {
    return list.map(nav => {
      if (isRoot) {
        ...
        if (nav.child) {
          return <FlexBox key={nav.title} alignItems="center" position="relative" flexDirection="column" sx={{
            "&:hover": {
              ...
              <FlexBox alignItems="flex-end" gap={0.3} sx={theme => navLinkStyle(theme)}>
                {nav.title}
              </FlexBox>
        ...
        if (nav.child) {
          return <ParentNav position="relative" minWidth="230px" key={nav.title}>
